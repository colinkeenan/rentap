...
if(typeof(ss.storage.rentapIDset) === 'undefined')
  ss.storage.rentapIDset = [true];
...
var rentapIDset = ss.storage.rentapIDset;
...
discardButton.PageMod({
   ...
   onAttach: function onAttach(worker) {
      worker.on('message', function(i,id) {
...
loadData.PageMod({
   ...
   onAttach: function onAttach(worker) {
      var rentaps = arrayofcsvToArrayofArrays(csvrentaps);
      var discards = arrayofcsvToArrayofArrays(csvdiscards);
      worker.postMessage([RHEADER,rentaps,discards,rentapByID,rentapIDset]);
