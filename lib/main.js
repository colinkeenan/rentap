   ...
   onAttach: function onAttach(worker) {
      worker.on('message', function(findname) {                //gets "findname" from searchButton.js
         if (findname === "showAllNames") {
            ...
            if (RegExp("\^\"").test(findname)) var regexp = RegExp("\^"+findname, "i");
            else var regexp = RegExp(findname, "ig");
            var i = 0;
            var foundnames = [[]];
            while (i < csv.length) {   //look for all csv data matching "findname" stored in simple-storage
               if (regexp.test(csv[i])) foundnames.push([i,csv[i]]);
               while(regexp.test(csv[i])); //for some reason, repeated tests are eventually false and if don't get to false, not all matches will be found
               i++;
            }   
            worker.postMessage(foundnames);       //sends matching csv data to the content script searchButton.js which stores it in sessionStorage
...
listSearchMenu.PageMod({
   ...
   onAttach: function onAttach(worker) {
      ...
      worker.on('message',function(message) {
         if(message==='click') worker.postMessage(csv);
