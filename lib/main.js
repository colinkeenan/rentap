...
if(typeof(ss.storage.csvrentaps) === 'undefined') { //initialize csvrentaps[0] to be an instruction page for renters to refer to when filling out an application
  ss.storage.csvrentaps = ['"My First M Last","###-##-####","mo/dy/year","Single/Divorced/Seperated/Married","emailname@emailprovider.com","driver\'s license/ID# and State","555-321-4321","555-123-1234","9080 Example Blvd, $200/mo' +
'\nCity, ST  Zip' +
'\nJun\'09 - present' +
...
if(typeof(ss.storage.discards) === 'undefined') {
  ss.storage.discards = [];
}

var csvrentaps = ss.storage.csvrentaps;
var RHEADER = ss.storage.RHEADER;
var discards = ss.storage.discards;

var CSV = require('./ucsvrentaps-1.1.0'); //UCSV v1.1.0 Provided under MIT License. Copyright 2010-2012, Peter Johnson http://www.uselesscode.org/javascript/csvrentaps/
...
delRentapButton.PageMod({
   ...
   onAttach: function onAttach(worker) {
      worker.on('message', function(i) {
         discards.splice(i,1);           //delButton is only visible on a discard
      });
   }
});
...
restoreButton.PageMod({
   ...
   onAttach: function onAttach(worker) {
      worker.on('message', function(i) {
         csvrentaps.push(discards[i]); //discard doesn't keep track of original row, so just restore to last row
         discards.splice(i,1);
      });
   }
});
