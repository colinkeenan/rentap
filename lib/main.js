...
var INITcsvdiscards = ['"Trash is Empty","","","","","","","","","","","","","","","","","","","","","No Trash Left"'];
var INIT = ['"Nothing to See Here","","","","","","","","","","","","","","","","","","","","",""'];
...
loadData.PageMod({
   ...
   onAttach: function onAttach(worker) {
      function arrayofcvsToArrayofArrays(arrayofcvs) {
         var arrayofArrays = [[]];
         for(var i=0; i<arrayofcvs.length; i++) {
            if (typeof(arrayofcvs[i]) != 'undefined' && arrayofcvs[i] != null) {
               arrayofArrays[i] = CSV.csvToArray(arrayofcvs[i])[0];
            } else {
               if (arrayofcvs.length <= 1) {
                  arrayofcvs[0] = INIT;
                  arrayofArrays[0] = CSV.csvToArray(arrayofcvs[0])[0];
               } else {
                  arrayofcvs.splice(i,1);
                  i--;
               }
            }
         }
      ...
      var rentaps = arrayofcvsToArrayofArrays(csvrentaps);
      var discards = arrayofcvsToArrayofArrays(csvdiscards);
