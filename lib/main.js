...
delRentapButton.PageMod({
   ...
   onAttach: function onAttach(worker) {
      worker.on('message', function(id) {
         csvrentaps[id] = null; //Not splicing it because want to preserve all the ID's
         trash.splice(indexOf(id),1); //Easier navigation of trash if remove ID of deleted rentap
      })
   }
});
